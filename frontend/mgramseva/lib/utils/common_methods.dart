...
class CommonMethods {
  ...
  static List<DatePeriod> getPastMonthUntilFinancialYear(int year){
    ...
    return monthList.map((e) => DatePeriod(DateTime(e.year, e.month, 1), DateTime(e.year, e.month + 1, 1), DateType.MONTH)).toList().reversed.toList();
  }
  ...
  static List<YearWithMonths> getFinancialYearList([int count = 5]){
    ...
    if(DateTime.now().month >= 4) {
      var year = DatePeriod(DateTime(DateTime.now().year, 4) , DateTime(DateTime.now().year + 1, 4), DateType.YTD);
    ...
    for(int i =0; i < count-1; i++){
      ...
      year = DatePeriod(DateTime(year.year - 1, 4), DateTime(year.year, 4), DateType.YEAR);
      var monthList = getPastMonthUntilFinancialYear(year.startDate.year);
      yearWithMonths.add(YearWithMonths(monthList, year));
    }
